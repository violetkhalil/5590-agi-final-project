...
def pack_backbone(model: nn.Module) -> torch.Tensor:
    return parameters_to_vector(model.parameters())


@torch.no_grad()
def unpack_backbone(model: nn.Module, vec: torch.Tensor):
    vector_to_parameters(vec, model.parameters())


def pack_mask(mask: GlobalMask) -> torch.Tensor:
    return mask.mask_logits.view(-1)


@torch.no_grad()
def unpack_mask(mask: GlobalMask, vec: torch.Tensor):
    mask.mask_logits.copy_(vec.view_as(mask.mask_logits))
...
def main():
    ...
    x = pack_backbone(backbone).detach().to(device)
    y = pack_mask(mask).detach().to(device)

    hparams = [x.clone().requires_grad_(True)]  # x ≡ θ_vec
    params = [y.clone().requires_grad_(True)]  # y ≡ φ_vec
    z = y.clone().requires_grad_(True)

    hparams_prior = [hparams[0].clone().detach().requires_grad_(True)]
    params_prior = [params[0].clone().detach().requires_grad_(True)]
    z_prior = z.clone().detach().requires_grad_(True)
    ...
    for o_step in range(num_epochs):
        step_start_time = time.time()

        # ============update x ==============
        W1_batch, X1_batch, y1_batch = next_batch_cycler(train_loader, it_state)
        W1_batch = W1_batch.to(device)
        X1_batch = X1_batch.to(device)
        y1_batch = y1_batch.to(device).float().view(-1)
        ...
        W2_batch, X2_batch, y2_batch = next_batch_cycler(train_loader, it_state)
        W2_batch = W2_batch.to(device)
        X2_batch = X2_batch.to(device)
        y2_batch = y2_batch.to(device).float().view(-1)
        loss = lower_loss(
            x_train_batch_2,
            y_train_batch_2,
            hparams[0],
