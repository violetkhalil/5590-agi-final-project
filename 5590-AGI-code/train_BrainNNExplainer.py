...
def main():
    ...
    ########################
    # ===== Step 1: Train backbone on original graphs =====
    opt_b = torch.optim.Adam(
        backbone.parameters(), lr=cfg["lr_backbone"], weight_decay=cfg["weight_decay"]
    )
    print("[Step 1] Train backbone on original graphs")

    best_val, best_state = -1, None
    for epoch in range(1, cfg["epochs_backbone"] + 1):
        tr_loss, tr_acc = run_epoch_backbone(
            backbone, train_loader, opt_b, device, train=True
        )
        va_loss, va_acc = evaluate(backbone, val_loader, device)
        print(
    ...
    test_loss, test_acc = evaluate(backbone, test_loader, device)
    print(f"[Backbone @orig] test: loss {test_loss:.4f} acc {test_acc:.3f}")

    ########################
    # ===== Step 2: Learn global mask M (freeze backbone) =====
    print("\n[Step 2] Learn global mask M with consistency + sparsity + entropy")
    mask = GlobalMask(cfg["n_nodes"]).to(device)
    for p in backbone.parameters():
        p.requires_grad_(False)
    opt_m = torch.optim.Adam(mask.parameters(), lr=cfg["lr_mask"])

    for epoch in range(1, cfg["epochs_mask"] + 1):
        loss_mask = run_epoch_mask(mask, backbone, train_loader, opt_m, device, cfg)
        print(f"Mask Epoch {epoch:03d} | loss {loss_mask:.4f}")
    # save mask
    M_learned = mask().detach().cpu()
    torch.save(M_learned, "mask_M.pt")
    print(
        "Saved learned mask to mask_M.pt. Mean=%.4f, sparsity(approx<=0.1)=%.3f"
        % (M_learned.mean().item(), (M_learned <= 0.1).float().mean().item())
    )

    ########################
    # ===== Step 3: Finetune backbone on masked graphs =====
    print("\n[Step 3] Finetune backbone on masked graphs")
    for p in backbone.parameters():
        p.requires_grad_(True)
    opt_b2 = torch.optim.Adam(
        backbone.parameters(), lr=cfg["lr_backbone"], weight_decay=cfg["weight_decay"]
    )

    # Wrap loaders to apply mask on-the-fly
    def masked_loader(base_loader: DataLoader):
        for batch in base_loader:
            ...
            with torch.no_grad():
                M = mask().to(device)
            Wm = GlobalMask.apply_mask(W, M)
            yield Batch(Wm, X, y)

    def run_masked_epoch(model, base_loader, optimizer, train=True):
        model.train(train)
        total_loss, total_acc, n = 0.0, 0.0, 0
        for batch in masked_loader(base_loader):
            logits = model(batch.W, batch.X)
            ...
            b = batch.y.size(0)
            total_loss += loss.item() * b
            total_acc += accuracy_from_logits(logits, batch.y) * b
            n += b
        return total_loss / n, total_acc / n

    best_val2, best_state2 = -1, None
    for epoch in range(1, cfg["epochs_finetune"] + 1):
        tr_loss, tr_acc = run_masked_epoch(backbone, train_loader, opt_b2, train=True)
        va_loss, va_acc = run_masked_epoch(backbone, val_loader, opt_b2, train=False)
        print(
            f"Finetune {epoch:03d} | train: loss {tr_loss:.4f}, acc {tr_acc:.3f} | val: loss {va_loss:.4f}, acc {va_acc:.3f}"
        )
        if va_acc > best_val2:
            best_val2, best_state2 = va_acc, {
                k: v.cpu().clone() for k, v in backbone.state_dict().items()
            }

    if best_state2 is not None:
        backbone.load_state_dict(best_state2)
    test_loss2, test_acc2 = run_masked_epoch(backbone, test_loader, opt_b2, train=False)
    print(f"[Backbone @masked] test: loss {test_loss2:.4f}, acc {test_acc2:.3f}")
