...
from Model.lrbgt import BrainNetworkTransformer


class Trainer_ALTER_CDR:
    """
    ...
    def _forward_batch(self, model, batch, device):
        ...
        num_nodes = self.conf.N
        feat_dim = batch.x.size(-1)
        expected_nodes = num_graphs * num_nodes
        if batch.x.size(0) != expected_nodes:
            raise ValueError(
                f"Unexpected node count: got {batch.x.size(0)}, "
                f"expected {expected_nodes} (num_graphs={num_graphs}, N={num_nodes})."
            )
        node_feature = batch.x.view(num_graphs, num_nodes, feat_dim)
        logits = model(node_feature, node_feature)
    ...
    def train(self):
        ...
        mode = getattr(self.conf, "pos_encoding", "rrwp")
        save_folder = os.path.join(save_folder, mode)
        ...
        nhead = 4
        use_rrwp = getattr(self.conf, "pos_encoding", "rrwp").lower() == "rrwp"
        if use_rrwp:
            rrwp_base = 16
            pos_embed_dim = rrwp_base + (nhead - (num_features + rrwp_base) % nhead) % nhead
            pos_encoding = "rrwp"
        else:
            pos_embed_dim = (nhead - (num_features % nhead)) % nhead
            pos_encoding = "identity" if pos_embed_dim > 0 else "none"
        bnt_config = OmegaConf.create({
            ...
            "model": {
                "pos_encoding": pos_encoding,  # ALTER=rrwp, BrainNetworkTransformer=identity
                "pos_embed_dim": int(pos_embed_dim),
                "sizes": [self.conf.N, 32, 8],
        ...
        backbone_model = BrainNetworkTransformer(bnt_config).to(device)

        opt_backbone = torch.optim.Adam(
            backbone_model.parameters(),
            lr=getattr(self.conf, "baseline_lr", 1e-4),
            weight_decay=getattr(self.conf, "baseline_weight_decay", 1e-4),
        )
