...
def add_every_rrwp(data, walk_length=8, add_identity=True, spd=False, **kwargs):
...
class BrainNetworkTransformer(BaseModel):
    ...
    def __init__(self, config: DictConfig):
        ...
        feature_dim = getattr(config.dataset, "feature_dim", None)
        if feature_dim is None:
            feature_dim = config.dataset.node_sz
        forward_dim = feature_dim
        ...
        if self.pos_encoding == "identity":
            ...
            forward_dim = feature_dim + config.model.pos_embed_dim
